// src/app/api/assessments/session/[sessionId]/current/route.ts
...
export async function GET(
  ...
  try {
    ...
    const currentTest = await orchestrator.getCurrentTest(sessionId)
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to get current test' }, 
      { status: 500 }
    )
  }
}